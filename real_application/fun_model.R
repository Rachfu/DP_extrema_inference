...
fun_linear_regression = function(D,Y,B,level,e,r,BS_true,k_par){
  ...
  lb_naive = beta_hat[s_max] - qt(1-level,df = n-1)*sqrt(1.0 * sigma_sq_hat * DtD_inv[s_max,s_max])
  lb_naive_whl = beta_hat_pri_whl[s_max] - qt(1-level,df = n-1)*sqrt(1.0 * sigma_sq_hat_pri * abs(solve(DtD+Lap_noise$whl_DtD)[s_max,s_max]))
  lb_naive_par = beta_hat_pri_par[s_max] - qt(1-level,df = n-1)*sqrt(1.0 * sigma_sq_hat_pri * abs(solve(DtD+Lap_noise$par_DtD)[s_max,s_max]))
  ...
  n_r = length(r)
  for (i in 1:n_r){
    d[i,] = (1-n^(r[i]-0.5))*(BS_hat-beta_hat)
    d_whl[i,] = (1-n^(r[i]-0.5))*(BS_hat_pri_whl-beta_hat_pri_whl)
    d_par[i,] = (1-n^(r[i]-0.5))*(BS_hat_pri_par-beta_hat_pri_par)
  }
  ...
  for (b in 1:B){
    ...
    for (i in 1:n_r){
      T_b[i,b] = sqrt(n)*(max(beta_boot+d[i,])-BS_hat)
      T_b_whl[i,b] = sqrt(n)*(max(beta_boot_whl+d_whl[i,])-BS_hat_pri_whl)
      T_b_par[i,b] = sqrt(n)*(max(beta_boot_par+d_par[i,])-BS_hat_pri_par)
    }
  }
